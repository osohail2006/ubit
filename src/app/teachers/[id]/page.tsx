  ...
  <div className="flex items-center justify-between py-1">
    ...
    <div className="flex gap-1">
      {[...Array(10)].map((_, i) => (
        <div
          key={i}
          className={`h-5 w-4 border-2 border-black ${i < score ? color : "bg-[#fffdf8]"}`}
        />
      ))}
    </div>
    <span className="w-8 text-right font-mono text-sm font-black">
      {score.toFixed(1)}
    </span>
  </div>
);
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
            {[
              ...
              "GRADING CURVE: Yes, generous",
              "HOMEWORK: Heavy",
            ].map((fact, i) => (
              <div key={i} className="border-2 border-black bg-gray-100 p-3">
                <span className="block font-mono text-[10px] font-bold uppercase text-gray-500">
                  {fact.split(":")[0]}
                </span>
          ...
          <div className="border-4 border-black bg-white p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <div className="mb-4 flex justify-between">
              <div className="flex gap-2">
                <span className="border-2 border-black bg-yellow-400 px-2 py-0.5 font-mono text-[10px] font-bold uppercase">
                  ENGAGING LECTURES
                </span>
              ...
              <div className="border-2 border-black bg-black px-2 py-0.5 font-mono text-sm font-black text-yellow-400">
                9.5
              </div>
            </div>
            <p className="mb-4 text-lg font-medium text-gray-800">
              Genuinely one of the best CS teachers on campus. Explains
              recursion like it's nothing. Office hours are actually useful.
            </p>
            <p className="mb-4 font-mono text-xs text-gray-500">
              Anonymous • Data Structures • Fall 2025 • 14 found helpful
            </p>

            {/* Teacher Reply */}
            <div className="border-l-4 border-black pl-4">
              <p className="font-mono text-[10px] font-bold uppercase tracking-widest">
                DR. AYESHA KHAN • TEACHER
              </p>
              <p className="text-sm font-medium text-gray-700">
                Glad the office hours are helping — bring your algorithm
                questions early in the week, it's less rushed!
              </p>
            </div>
          </div>
              ...
              <div className="space-y-6">
                <div>
                  <ModalRatingSelector
                    ...
                    score={teachingScore}
                    setScore={setTeachingScore}
                    color="bg-blue-500"
                  />
                  <ModalRatingSelector
                    label="Paper Grading"
                    score={gradingScore}
                    setScore={setGradingScore}
                    color="bg-red-500"
                  />
                  <ModalRatingSelector
                    label="Attendance Leniency"
                    score={leniencyScore}
                    setScore={setLeniencyScore}
                    color="bg-green-500"
                  />
                  <ModalRatingSelector
                    label="Friendliness"
                    score={friendlinessScore}
                    setScore={setFriendlinessScore}
                    color="bg-yellow-400"
                  />
                </div>
                ...
                {/* Tags Selector */}
                <div>
                  <label className="mb-2 block font-mono text-xs font-bold uppercase tracking-widest">
                    Select up to 3 tags
                  </label>
